...
test('step 3 pre-saves account run record with password after fill succeeds', async () => {
  const events = {
    records: [],
    stateUpdates: [],
  };
  ...
  const executor = api.createStep3Executor({
    addLog: async () => {},
    appendAccountRunRecord: async (status, recordState) => {
      events.records.push({ status, recordState });
    },
    chrome: { tabs: { update: async () => {} } },
    ensureContentScriptReadyOnTab: async () => {},
    generatePassword: () => 'Generated123!',
  ...
  await executor.executeStep3({
    ...
    customPassword: 'SavedSecret123!',
    accounts: [],
  });

  assert.equal(events.stateUpdates[0].accounts[0].password, 'SavedSecret123!');
  assert.deepStrictEqual(events.records, [
    {
      status: 'running',
      recordState: {
        email: 'save@example.com',
        ...
        accounts: [],
        accountIdentifierType: 'email',
        accountIdentifier: 'save@example.com',
        phoneNumber: '',
